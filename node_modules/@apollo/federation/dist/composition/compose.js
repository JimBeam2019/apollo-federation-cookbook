        ...
        for (let definition of typeDefsWithoutExternalFields.definitions) {
            ...
            if (graphql_1.isTypeDefinitionNode(definition)) {
                ...
                if (definition.kind === graphql_1.Kind.OBJECT_TYPE_EXTENSION ||
                    ...
                    if (typeToServiceMap[typeName]) {
                        typeToServiceMap[typeName].extensionFieldsToOwningServiceMap = Object.assign({}, typeToServiceMap[typeName].extensionFieldsToOwningServiceMap, fields);
                    }
                    else {
                        typeToServiceMap[typeName] = {
                ...
                if (definition.kind === graphql_1.Kind.ENUM_TYPE_EXTENSION) {
                    ...
                    if (typeToServiceMap[typeName]) {
                        typeToServiceMap[typeName].extensionFieldsToOwningServiceMap = Object.assign({}, typeToServiceMap[typeName].extensionFieldsToOwningServiceMap, values);
                    }
                    else {
                        typeToServiceMap[typeName] = {
            ...
            definitionsMap[extensionTypeName] = [
                {
                    ...
                    name: { kind: graphql_1.Kind.NAME, value: extensionTypeName },
                    fields: [],
                },
            ];
...
function addFederationMetadataToSchemaNodes({ schema, typeToServiceMap, externalFields, keyDirectivesMap, }) {
    for (const [typeName, { serviceName: baseServiceName, extensionFieldsToOwningServiceMap },] of Object.entries(typeToServiceMap)) {
        const namedType = schema.getType(typeName);
        namedType.federation = Object.assign({}, namedType.federation, { serviceName: baseServiceName }, (keyDirectivesMap[typeName] && {
        ...
        if (graphql_1.isObjectType(namedType)) {
            for (const field of Object.values(namedType.getFields())) {
                ...
                if (providesDirective &&
                    ...
                    field.federation = Object.assign({}, field.federation, { provides: utils_1.parseSelections(providesDirective.arguments[0].value.value) });
                }
            }
        }
        for (const [fieldName, extendingServiceName] of Object.entries(extensionFieldsToOwningServiceMap)) {
            if (graphql_1.isObjectType(namedType)) {
                ...
                field.federation = Object.assign({}, field.federation, { serviceName: extendingServiceName });
                ...
                if (requiresDirective &&
                    ...
                    field.federation = Object.assign({}, field.federation, { requires: utils_1.parseSelections(requiresDirective.arguments[0].value.value) });
                }
            }
        }
    }
    for (const field of externalFields) {
        ...
        if (!namedType)
            continue;
        namedType.federation = Object.assign({}, namedType.federation, { externals: Object.assign({}, (namedType.federation && namedType.federation.externals), { [field.serviceName]: [
...
function composeServices(services) {
    const { typeToServiceMap, definitionsMap, extensionsMap, externalFields, keyDirectivesMap, } = buildMapsFromServiceList(services);
    ...
    schema = apollo_graphql_1.transformSchema(schema, type => {
        if (graphql_1.isObjectType(type)) {
            ...
            return new graphql_1.GraphQLObjectType(Object.assign({}, config, { interfaces: Array.from(new Set(config.interfaces)) }));
        }
        return undefined;
    });
